...
/*
 * returns a promise resolving to apilist
 */
async function DiscoverApi()
{
  let arr = (await nodeapi({cmd:"DiscoverApi"}));
  if( typeof(arr) == "string" ) arr = jsp(arr);
  
  anci.apilist = arr[0] ;
  return arr[0];
}

var lsapi = DiscoverApi ;
anci._absorb( lsapi, "lsapi" ) ;

anci._autoAddApi = async function()
{
  let apilist = await DiscoverApi();

  for(let func in apilist)
  {
    let args = apilist[func];
    ...
    if(!anci[func])
    {
      ...
      {
          let fname = args[0][0] ;
          anci[func] = anci[ fname ] ;
          if( anci[func] )
            anci[func].Name = fname ;
            
          continue;
      }  
      
      let argswoeq = args.map(i=>i.split("=")[0]) ;
        
      let asyncFunction = Object.getPrototypeOf( async()=>{} ).constructor ;
      anci[func] = new asyncFunction(...args,
      `
        let arr = (await anci.currentapi(
          {
            cmd:"${func}",
            ${argswoeq.toString()}
          }
...
function GetDocumentation(outputHTML,filterString, testButton=true)
{
  ...
  arr=arr.map(ii=>
    {
	    let par=(anci[ii].toString().match(/\([\s\S]*?\)/) || [])[0] || "";
...
(async ()=>{
  anci.server_running = await anci.hasurl("/isthisananciserver") ;
  if(anci.server_running || anci.platform == "android" )
  {
    await anci._autoAddApi();
...
try{
  ...
  anci.currentlyMovedApi = `
    hasf, hasd, rm, rmd/rm_r, mkdir, rf, rfb, realp
  ` ;
